...
def load_model(args, exp_name: str, exp_id: str, device: Union[torch.device, str] = 'cpu'):
    ...
    for arg, model_arg in zip(reversed(run_args), reversed(run_model_args)):
        if arg.base_on is None:
        ...
        elif arg.model_class == 'acm':
            # TODO refactor this and instantiation of ACM models so that no duplication here is needed
            from architectures.acm import acmize_vit
            from architectures.custom import simplify_mha
            simplify_mha(model)
            dataset_args = {} if 'dataset_args' not in arg or arg.dataset_args is None else arg.dataset_args
            _, train_eval_loader, _ = DATASETS_NAME_MAP[arg.dataset](dataset_args)
            example_input, _ = next(iter(train_eval_loader))
            example_input = example_input.unsqueeze(0).to(device, non_blocking=True)
            model, _ = acmize_vit(model, example_input=example_input, **model_arg)
            model = model.to(device)
        elif 'facm' in arg.model_class:
            model = model.to(device)
        elif 'acm' in arg.model_class:
            # TODO refactor this and instantiation of ACM models so that no duplication here is needed
            from architectures.acm import add_gating_networks
            add_gating_networks(model, arg.model_class, model_arg)
            model = model.to(device)
            ...
            model, _ = replace_with_moes(model, **model_arg)
            model = model.to(device)
