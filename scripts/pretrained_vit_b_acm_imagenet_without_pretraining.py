...
def main():
    ...
    for epochs_distill, epochs_gating, epochs_e2e, display_name in [
        (2, 1, 97, 'With pre-training'),
        (0.0, 1, 99, 'Without phase I'),
        (0, 0, 100, 'Without pre-training'),
    ]:
        ...
        for opt_class, bs, lr, wd, dropout, bias, detach, ffn, o_proj, q_proj, k_proj, v_proj, total_flops_factor, num_blocks in [
            ('adam', 64, 0.001, 0.0, 0.0, 'hidden_only', 'no_detach', True, True, True, True, True, 1.0, 4),
        ]:
            for exp_id in exp_ids:
                args = deepcopy(acm_args)
                args.base_on = base_exp_name
                args.exp_id = exp_id
                args.epochs = epochs_distill
                args.batch_size = bs
        ...
        for base_on_exp_name in base_acm_exp_names:
            for gating_type, gating_model_args, bs, opt_class, lr, gating_error_factor in [
                ('acm_deep_gumbel', {'gs_tau': 0.8, 'min_choice': min_choice_dict,
                                     'depth': 3, 'gating_hidden_dim': 16}, 64, 'adam', 0.01, 1.2),
            ]:
                for exp_id in exp_ids:
        ...
        for base_on_exp_name in base_gacm_exp_names:
            for bs, optimizer_class, lr, routing_loss_type, routing_loss_weight, routing_loss_target, \
                    entropy_loss_weight, compute_diversity_loss_weight in [
                (512, 'adam', 1e-4, 'l1', 1e-1, 0.60, 0.05, 0.05),
            ]:
                for exp_id in exp_ids:
                    args = deepcopy(finetune_args)
                    args.base_on = base_on_exp_name
                    args.exp_id = exp_id
                    args.epochs = epochs_e2e
                    args.eval_points = epochs_e2e // 2 + 1
                    args.batch_size = bs
    ...
    out_dir_name = f"{common_args.dataset}_pretrained_vit_b_pretraining_ablation"
