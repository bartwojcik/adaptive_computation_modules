...
def main():
    ...
    for base_on_exp_name in base_gacm_exp_names:
        for epochs, bs, optimizer_class, lr, routing_loss_type, routing_loss_weight, routing_loss_target, \
            ...
            (5, 256, 'adam', 1e-4, 'l1', 1e-1, 0.60, 0.0, 0.05), # no sequence entropy loss
            (5, 256, 'adam', 1e-4, 'l1', 1e-1, 0.60, 0.05, 0.0), # no compute diversity loss
            (5, 256, 'adam', 1e-4, 'l1', 1e-1, 0.60, 0.0, 0.0), # no sequence entropy nor compute diversity loss
            (5, 256, 'adam', 1e-4, 'l1', 0.0, 0.60, 0.05, 0.05), # no budget loss
        ]:
            for exp_id in exp_ids:
    ...
    out_dir_name = f"{common_args.dataset}_pretrained_vit_b_loss_ablation"
