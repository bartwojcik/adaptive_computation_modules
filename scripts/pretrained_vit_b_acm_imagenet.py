...
from visualize.acm_distillation_losses import get_default_args as get_default_distill_plot_args
from visualize.acm_distillation_losses import main as distillation_losses_plot
from visualize.acm_gating_distribution import get_default_args as get_default_gating_distribution_plot_args
from visualize.acm_gating_distribution import main as gating_distribution_plot
from visualize.acm_image_spatial_load import get_default_args as get_default_image_spatial_load_args
from visualize.acm_image_spatial_load import main as spatial_load_plot
...
def main():
    ...
    # cascading_model_args.model_args.head_type = 'vit_cascading_head'
    ...
    executor.update_parameters(slurm_gpus_per_task=f'{gpu_type}{avit_gpus_per_task}')
    for lr, bs, distr_target_depth, gate_scale, gate_center, ponder_weight, distr_weight in [
        ...
        for exp_id in exp_ids:
            ...
            if base_run_name in run_to_job_map:
                ...
                executor.update_parameters(slurm_additional_parameters={})
            job = submit_job(executor, train_avit, args, num_gpus=avit_gpus_per_task)
            jobs.append(job)
            exp_name, run_name = generate_run_name(args)
            run_to_job_map[run_name] = job
        exp_names.append(exp_name)
    ...
    relufication_args.epochs = 80
    # relufication_args.epochs = 20
    # relufication_args.epochs = 0.1
    relufication_args.batch_size = 512
    # relufication_args.batch_size = 256
    # relufication_args.batch_size = 128
    relufication_args.eval_points = 10
    ...
    for exp_id in exp_ids:
        ...
        run_to_job_map[run_name] = job
    # exp_names.append(exp_name)
    base_relufication_exp_names.append(exp_name)
    ...
    for base_on_exp_name in base_relufication_exp_names:
        for exp_id in exp_ids:
            ...
            run_to_job_map[run_name] = job
        # exp_names.append(exp_name)
        base_moefication_exp_names.append(exp_name)
    ...
    # moefication_routing_args.epochs = 20
    moefication_routing_args.epochs = 10
    # moefication_routing_args.epochs = 0.01
    moefication_routing_args.batch_size = 512
    # moefication_routing_args.batch_size = 256
    # moefication_routing_args.batch_size = 128
    ...
    moefication_routing_args.k_to_eval = [2, 4, 8, 12, 16, 24, 32, 48, 64, 96, 128]
    ...
    # moefication_routing_args.mixed_precision = None
    ...
    for base_on_exp_name in base_moefication_exp_names:
        for exp_id in exp_ids:
            ...
            run_to_job_map[run_name] = job
        exp_names.append(exp_name)
        base_routed_moefication_exp_names.append(exp_name)
    ...
    for opt_class, bs, lr, wd, dropout, bias, detach, ffn, o_proj, q_proj, k_proj, v_proj, total_flops_factor, num_blocks, epochs in [
        ...
        # FFNs only
        # ('adam', 64, 0.001, 0.0, 0.0, 'hidden_only', 'no_detach', True, False, False, False, False, 1.0, 4, 2),
    ]:
        for exp_id in exp_ids:
            ...
            # display_names.append(f'ACM (phase I)')
            base_acm_display_names.append(f'ACM (phase I)')
    ...
    for base_on_exp_name in base_acm_exp_names:
        for gating_type, gating_model_args, epochs, bs, opt_class, lr, gating_error_factor in [
            ...
            for exp_id in exp_ids:
                ...
                # display_names.append(f'ACM (phase II)')
    ...
    for base_on_exp_name in base_gacm_exp_names:
        for epochs, bs, optimizer_class, lr, routing_loss_type, routing_loss_weight, routing_loss_target, \
            ...
            (97, 512, 'adam', 1e-4, 'l1', 1e-1, 0.75, 0.05, 0.05),
            (97, 512, 'adam', 1e-4, 'l1', 1e-1, 0.60, 0.05, 0.05),
            (97, 512, 'adam', 1e-4, 'l1', 1e-1, 0.40, 0.05, 0.05),
            (97, 512, 'adam', 1e-4, 'l1', 1e-1, 0.25, 0.05, 0.05),
        ]:
            for exp_id in exp_ids:
                ...
                run_to_job_map[run_name] = job
                exp_names.append(exp_name)
                # display_names.append(f'ACM (b={routing_loss_target} bw={routing_loss_weight}'
                #                      f'ew={entropy_loss_weight} dw={compute_diversity_loss_weight})')
                display_names.append(f'ACM (budget target {routing_loss_target})')
    ...
    out_dir_name = f"{common_args.dataset}_pretrained_vit_b"
    ...
    # ════════════════════════ ACM representation distillation analysis ════════════════════════ #

    selected_pairs = [(exp_name, display_name) for exp_name, display_name
                      in zip(base_acm_exp_names, base_acm_display_names)
                      if 'acm_' in exp_name]
    selected_exp_names, selected_display_names = zip(*selected_pairs)

    plot_args = get_default_distill_plot_args()
    plot_args.output_dir = output_dir
    plot_args.runs_dir = common_args.runs_dir
    plot_args.exp_names = selected_exp_names
    plot_args.exp_ids = exp_ids
    plot_args.display_names = selected_display_names
    # plot_args.batches = 0
    plot_args.batches = 200
    plot_args.batch_size = 32
    plot_args.use_wandb = False
    submit_job(executor, distillation_losses_plot, plot_args)

    # ════════════════════════ ACM gating distribution analysis ════════════════════════ #

    selected_pairs = [(exp_name, display_name) for exp_name, display_name in zip(exp_names, display_names) if
                      'acm_' in exp_name and 'phase I' not in display_name]
    selected_exp_names, selected_display_names = zip(*selected_pairs)

    plot_args = get_default_gating_distribution_plot_args()
    plot_args.output_dir = output_dir
    plot_args.runs_dir = common_args.runs_dir
    plot_args.exp_names = selected_exp_names
    plot_args.exp_ids = exp_ids
    ...
    # plot_args.batches = 0
    plot_args.batches = 200
    plot_args.batch_size = 64
    plot_args.use_wandb = False
    submit_job(executor, gating_distribution_plot, plot_args)

    # ════════════════════════ ACM spatial computational load analysis ════════════════════════ #

    selected_pairs = [(exp_name, display_name) for exp_name, display_name in zip(exp_names, display_names) if
                      'acm_' in exp_name and 'phase I' not in display_name]
    selected_exp_names, selected_display_names = zip(*selected_pairs)

    plot_args = get_default_image_spatial_load_args()
    plot_args.output_dir = output_dir
    plot_args.runs_dir = common_args.runs_dir
    plot_args.exp_names = selected_exp_names
    plot_args.exp_ids = exp_ids
    ...
    plot_args.batch_size = 64
    plot_args.use_wandb = False
    submit_job(executor, spatial_load_plot, plot_args)
